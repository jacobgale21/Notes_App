...
async def generate_graph_endpoint(body: GenerateGraphRequest, db: Session = Depends(get_db),current_user: UserModel = Depends(get_current_user)):
    graph_id = generate_graph(db, current_user, body.notes)
